#!/usr/bin/env python3
"""
TikTok Standard flow (PHONE_002):
...
def shell_output(d: u2.Device, cmd: str) -> str:
    ...
    if isinstance(result, tuple):
        # uiautomator2 may return (output, exit_code)
...
def accept_vpn_dialog_if_present(d: u2.Device) -> bool:
    # Android VPN confirmation dialog package: com.android.vpndialogs
    ...
    selectors = [
        ...
        d(textMatches=r"(?i)^(allow|ok|cho phép|dong y|đồng ý)$"),
        d(descriptionMatches=r"(?i)^(allow|ok|cho phép|dong y|đồng ý)$"),
    ]
    ...
    # Fallback tap on positive button area.
    w, h = d.window_size()
...
def ensure_socksdroid_fake_ip_ready(d: u2.Device, max_attempts: int = 3) -> None:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if current_package(d) != SOCKS_PKG:
            log("SocksDroid gate: app not foreground yet, continue recovery")
        ...
        if switch_sel.exists:
            ...
            # Scale fallback tap from known MuMu base coordinates.
            w, h = d.window_size()
            ...
            tap_point_stable(
                ...
                x,
                y,
                label="socksdroid switch fallback",
                retries=2,
                post_wait=(0.7, 1.2),
            )
    ...
    raise DevicePrepError(
        "SocksDroid fake IP is not active. Stop flow by rule: do not open TikTok without fake IP ON."
    )
...
def handle_anr_dialog(d: u2.Device) -> bool:
    ...
    # Prefer closing the stuck app first for faster recovery.
    for sel in [
    ...
    # Fallback to "Wait" only when close button is unavailable.
    for sel in [
...
def ensure_app_front(d: u2.Device, package_name: str = PKG, retries: int = 3) -> None:
    for attempt in range(1, retries + 1):
        handle_anr_dialog(d)
        if dismiss_google_signin_sheet(d):
            handle_anr_dialog(d)
        ...
        # Critical rule: never open TikTok before SocksDroid fake IP is active.
        if package_name == PKG:
...
def dismiss_feed_coachmark_if_present(d: u2.Device) -> bool:
    """
    Dismiss first-run coachmark overlays on TikTok feed
    (e.g., 'Vuốt lên để xem thêm') that intercept taps on Search.
    """
    if current_package(d) != PKG:
        return False

    coachmark_selectors = [
        d(textMatches=r"(?i).*vu\u1ed1t l\u00ean.*xem th\u00eam.*"),
        d(textMatches=r"(?i).*swipe up.*more.*"),
        d(textMatches=r"(?i).*\u0111\u1ec3 xem th\u00eam.*"),
    ]
    visible = any(sel.exists for sel in coachmark_selectors)
    if not visible:
        return False

    log("coachmark overlay detected; dismiss by swipe up")
    w, h = d.window_size()
    d.swipe(int(w * 0.50), int(h * 0.78), int(w * 0.50), int(h * 0.28), 0.24)
    sleep_rand(1.0, 1.6)
    return True
    ...
    for nav_attempt in range(1, 4):
        ...
        if not warmup_done and PRE_SEARCH_WARMUP_SWIPES > 0:
            ...
            if current_package(d) != PKG:
                ...
                continue
        dismiss_feed_coachmark_if_present(d)
...
def count_right_action_nodes(d: u2.Device) -> int:
    ...
    xml = d.dump_hierarchy(compressed=False, pretty=False)
    matches = re.finditer(
        rf'class="([^"]+)"[^>]*package="{PKG_REGEX}"[^>]*clickable="true"[^>]*bounds="(\[[^"]+\])"',
        xml,
    )
    count = 0
    for m in matches:
...
def get_result_tile_points(d: u2.Device) -> list[tuple[int, int]]:
    xml = d.dump_hierarchy(compressed=False, pretty=False)
    nodes = re.finditer(
        rf'class="android\.view\.ViewGroup"[^>]*package="{PKG_REGEX}"[^>]*clickable="true"[^>]*bounds="(\[[^"]+\])"',
        xml,
    )
...
def recover_back_to_search_results(d: u2.Device) -> None:
    # Avoid blind multiple BACK presses because they can exit to launcher.
    for _ in range(2):
        if current_package(d) != PKG:
            return
        if find_search_box(d) is not None:
        ...
        if any(k in act for k in ["DetailActivity", "SparkActivity", "Comment", "Profile"]):
            d.press("back")
            sleep_rand(0.7, 1.2)
            continue
        return
...
def open_first_visible_video(d: u2.Device) -> None:
    ...
    for idx, (x, y) in enumerate(points, start=1):
        ...
        handle_anr_dialog(d)
        if current_package(d) != PKG:
            raise RuntimeError("Lost TikTok context while opening first visible video")
...
def main() -> int:
    ...
    parser.add_argument(
        "--evidence-dir",
        default="",
        help="Directory to save per-step screenshots/xml evidence. Default: disabled.",
    )
    parser.add_argument(
        "--delay-multiplier",
        type=float,
    ...
    # Keep selector wait short; long implicit waits make every exists() check
    # stall for many seconds and look like the script is frozen.
