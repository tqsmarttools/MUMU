...
def open_tiktok(d: u2.Device) -> None:
    ...
    for attempt in range(1, 4):
        ...
        cur = d.app_current()
        if cur.get("package", "") == PKG and (home_tab_visible(d) or "SplashActivity" not in cur.get("activity", "")):
            return
        ...
        if wait_package_foreground(d, PKG, timeout_sec=14.0, stable_sec=1.1):
            # Give app extra settle time and wait for splash to pass.
            settle_deadline = time.time() + 14
            while time.time() < settle_deadline:
                ...
                if cur.get("package", "") != PKG:
                    break
                if home_tab_visible(d) or ("SplashActivity" not in cur.get("activity", "")):
                    human_sleep(1.6, 2.4)
                    return
                human_sleep(0.8, 1.2)
